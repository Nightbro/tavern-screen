...
function makeTmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'tavern-lib-'));
}

/** In-memory config mock (no file I/O needed in library tests) */
...
describe('setRootFolder', () => {
  ...
  test('persists root folder via config.set', () => {
    ...
    fs.mkdirSync(root);
    lib.setRootFolder(root);
    expect(cfg._data.rootFolder).toBe(root);
  });

  test('restores root folder from config on init', () => {
    const { tmp } = setup();
    const root = path.join(tmp, 'Persisted');
    fs.mkdirSync(root);

    const cfg1 = makeConfig();
    createLibrary(cfg1).setRootFolder(root);

    // New library instance with the same stored value
    const lib2 = createLibrary(makeConfig({ rootFolder: cfg1._data.rootFolder }));
  ...
  test('does not restore a root folder that no longer exists', () => {
    ...
    const lib = createLibrary(makeConfig({ rootFolder: fakePath }));
...
describe('scan', () => {
  ...
  test('returns projects from subdirectories', () => {
    const { lib, tmp } = setup();
    const root = path.join(tmp, 'R');
    writeImage(path.join(root, 'maps', 'Session 1'), 'map.jpg');
    lib.setRootFolder(root);
  ...
  test('map id is relative path from mapsDir', () => {
    const { lib, tmp } = setup();
    const root = path.join(tmp, 'R');
    writeImage(path.join(root, 'maps', 'S1'), 'map.jpg');
    lib.setRootFolder(root);
...
describe('copyFiles', () => {
  test('copies an image into maps/ root', () => {
    const { lib, tmp } = setup();
    lib.setRootFolder(path.join(tmp, 'R'));
  ...
  test('copies image into a project subdirectory', () => {
    const { lib, tmp } = setup();
    lib.setRootFolder(path.join(tmp, 'R'));
